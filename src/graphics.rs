...
pub(crate) fn background(canvas: &mut Canvas<Window>) {
    ...
    // background
    canvas.fill_rect(Rect::new(0, 0, 800, 600)).expect(&*format!("Can't draw background with width = {} and height = {}", 800, 600));
}
...
pub(crate) fn draw_circle(canvas: &mut Canvas<Window>, center: Point, radius: i32, color: Color)
{
    ...
    for x in (x0 - radius)..(x0 + 1) {
        for y in y0..(y0 + radius) {
            ...
            canvas.draw_point(Point::new(x, y)).expect(&*format!("Can't draw point [{},{}]", x, y));
            canvas.draw_point(Point::new(x, 2 * y0 - y)).expect(&*format!("Can't draw point [{},{}]", x, 2 * y0 - y));
            canvas.draw_point(Point::new(2 * x0 - x, y)).expect(&*format!("Can't draw point [{},{}]", 2 * x0 - x, y));
            canvas.draw_point(Point::new(2 * x0 - x, 2 * y0 - y)).expect(&*format!("Can't draw point [{},{}]", 2 * x0 - x, 2 * y0 - y));
        }
    }
}
